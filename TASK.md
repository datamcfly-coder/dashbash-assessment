...
1. **The number Dana checks every Monday.** She asks her operations lead "how many calls did we connect last week?" That number — calls whose `outcome` is `connected` and whose `started_at` falls within the last 7 days — must appear clearly somewhere on the dashboard, and must come from a live query against the database. If it's wrong or hardcoded, the rest doesn't matter.
...
The dataset used during review may differ from the one generated by `pnpm seed`. Your queries should produce correct results regardless of which dataset is loaded.
